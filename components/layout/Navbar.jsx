...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      <div className={`mx-auto w-full px-5 sm:px-9 md:px-10 xl:px-12 max-w-[1536px] relative flex shrink-0 items-center justify-between transition-colors duration-300 ${isMobileMenuOpen ? 'h-header border-b border-gray-200 bg-white' : 'h-header bg-transparent'
        }`}>
                ...
                <li key={index}>
                  {item.type === 'dropdown' ? (
                    <button className="group relative flex h-14 select-none items-center px-[18px] text-body-400 no-underline outline-hidden before:absolute before:inset-x-0 before:inset-y-1.5 before:rounded-full before:bg-cta-primary-bg before:opacity-0 hover:text-cta-primary-text hover:before:opacity-100">
                      <span className="relative flex items-center gap-1">{item.label}
                        <svg viewBox="0 0 12 12" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="relative mt-0.5 size-3 transition-transform duration-200 ease-out">
                    ...
                    <Link href={item.href} className="group relative flex h-14 select-none items-center px-[18px] text-body-400 no-underline outline-hidden before:absolute before:inset-x-0 before:inset-y-1.5 before:rounded-full before:bg-cta-primary-bg before:opacity-0 hover:text-cta-primary-text hover:before:opacity-100">
        ...
        <button
          ...
          className={`[&_svg]:[stroke-width-2] inline-flex items-center justify-center gap-2 rounded-sm font-sans text-[1rem] leading-normal tracking-[-0.002em] no-underline transition-colors duration-200 hover:no-underline focus-visible:outline-1 focus-visible:ring-1 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-60 [&_svg]:pointer-events-none [&_svg]:size-5 [&_svg]:shrink-0 size-10 p-0 hover:bg-black/10 translate-x-2 sm:translate-x-4 md:hidden z-20 ${isMobileMenuOpen ? 'text-ink-1 bg-transparent' : 'text-surface-text-primary bg-transparent'
            }`}
        >
          {isMobileMenuOpen ? (
      ...
      {isMobileMenuOpen && (
        <div className="flex flex-col flex-1 bg-white px-5 sm:px-9 overflow-y-auto w-full md:hidden">
          <nav className="flex flex-col w-full">
            {navItems.map((item, index) => (
              item.type === 'dropdown' ? (
                <button key={index} className="flex justify-between items-center py-5 border-b border-gray-100 text-ink-1 hover:text-purple-700 transition-colors">
